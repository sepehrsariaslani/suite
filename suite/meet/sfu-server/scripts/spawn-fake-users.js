...
const argv = yargs(hideBin(process.argv))
  .option('meeting', { type: 'string', demandOption: true, describe: 'Meeting ID / room ID to join' })
  .option('count', { type: 'number', default: 5, describe: 'Number of fake users to spawn' })
  .option('sfu-url', { type: 'string', default: 'http://localhost', describe: 'Base SFU URL (protocol+host)' })
  .option('sfu-port', { type: 'number', default: 3000, describe: 'SFU port if not implicit in URL' })
  .option('secret', { type: 'string', default: process.env.JWT_SECRET || 'fallback-secret', describe: 'JWT signing secret used by SFU' })
  .option('with-producers', { type: 'boolean', default: false, describe: 'Create mock audio/video producers' })
  .option('auto-toggle', { type: 'boolean', default: false, describe: 'Periodically send media_control events' })
  .option('lifetime', { type: 'number', default: 0, describe: 'Milliseconds before disconnecting (0 = keep alive)' })
  .help()
  .argv;
...
async function spawnUser(index) {
  ...
  socket.on('disconnect', (reason) => console.log(`[#${index}] 🔌 Disconnected: ${reason}`));
  socket.on('connect_error', (err) => console.error(`[#${index}] ❌ Connect error:`, err.message));

  socket.on('participant_joined', (d) => console.log(`[#${index}] 👥 participant_joined`, d.participantId));
  socket.on('participant_left', (d) => console.log(`[#${index}] 👋 participant_left`, d.participantId));
  socket.on('producer_created', (d) => console.log(`[#${index}] 🎥 producer_created`, d.producerId, d.kind));
  socket.on('media_control_update', (d) => console.log(`[#${index}] 🎛️ media_control_update`, d.action, 'from', d.participantId));
    ...
    socket.on('connect', () => {
      setTimeout(() => {
        socket.emit('get_router_rtp_capabilities', {}, (resp) => {
          if (!resp?.success) return console.warn(`[#${index}] Failed to get RTP caps`);
          // Create send transport
          socket.emit('create_webrtc_transport', { direction: 'send' }, (tResp) => {
            if (!tResp?.success) return console.warn(`[#${index}] Failed to create transport`);
            ...
            // Try audio producer
            socket.emit('create_producer', { transportId, rtpParameters: mockRtpParameters, kind: 'audio', appData: { type: 'mock' } }, (pResp) => {
              if (!pResp?.success) return console.warn(`[#${index}] Could not create audio producer (expected if server validates DTLS)`);
              console.log(`[#${index}] ✅ Mock audio producer created`, pResp.id);
            });
            ...
            const videoRtpParameters = {
              ...
              encodings: [{ ssrc: Math.floor(Math.random()*1e9), active: true }],
              rtcp: { cname: 'fake', reducedSize: true }
            };

            socket.emit('create_producer', { transportId, rtpParameters: videoRtpParameters, kind: 'video', appData: { type: 'camera' } }, (vResp) => {
              if (!vResp?.success) return console.warn(`[#${index}] Could not create video producer (expected if DTLS enforcement)`);
              console.log(`[#${index}] ✅ Mock video producer created`, vResp.id);
            });
          });
        });
