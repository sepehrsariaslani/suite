...
describe('pushNotificationConfig', () => {
	...
	it('rejects missing or malformed firebase web configs', () => {
		...
		expect(isValidWebConfig({ ...validConfig, apiKey: '' })).toBe(false)
	})
	...
	it('only builds a service worker URL when config is valid', () => {
		expect(buildServiceWorkerURL('/assets/suite/frontend/sw.js', validConfig)).toContain('?config=')
	...
	it('detects stale invalid mail service worker registrations', () => {
		const invalid = 'https://dehati.ir/assets/suite/frontend/sw.js?config=undefined'
		expect(isMailServiceWorkerScriptURL(invalid)).toBe(true)
		expect(isInvalidServiceWorkerScriptURL(invalid)).toBe(true)
		expect(
			isInvalidServiceWorkerScriptURL(
				'https://dehati.ir/assets/suite/frontend/sw.js?config=null',
			),
		).toBe(true)
		expect(isMailServiceWorkerScriptURL('https://dehati.ir/assets/other/sw.js')).toBe(false)
	})
})
