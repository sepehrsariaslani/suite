...
// snapshots (Sheet Snapshot). See spreadsheet/versioning/ for the
...
const PREFIX = 'spreadsheet.versioning.api'
...
export async function makeACopy(sheet, version, title = '') {
	const state = await call(`${PREFIX}.state_at`, { snapshot: version })
	const out = await call('spreadsheet.api.save_sheet', {
		title:       title || 'Copy',
...
export function recordOp({ sheet, opType, cellRefs = null, before = null,
	...
	return call('spreadsheet.api.record_op', {
		sheet,
		op_type:   opType,
