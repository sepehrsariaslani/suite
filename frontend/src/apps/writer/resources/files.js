...
import { openEntity, setTitle } from '@/utils'
// import store from '@/store'
import router from '@/router'
import { prettyData, setCache } from '@/utils'
...
export const getTeams = createResource({
  url: '/api/method//api/method/drive.api.permissions.get_teams',
  params: {
    details: 1,
  },
  method: 'GET',
...
export const toggleFav = createResource({
  url: '/api/method/drive.api.files.set_favourite',
  makeParams(data) {
    if (!data) {
...
export const clearRecent = createResource({
  url: '/api/method/drive.api.files.remove_recents',
  makeParams: (data) => {
    if (!data) {
      getRecents.setData([])
      return { clear_all: true }
...
export const clearTrash = createResource({
  url: '/api/method/drive.api.files.delete_entities',
  makeParams: (data) => {
    if (!data) {
      getTrash.setData([])
      return { clear_all: true }
...
export const move = createResource({
  url: '/api/method/drive.api.files.move',
  onSuccess(data) {
    toast({
...
export const translate = createResource({
  method: 'GET',
  url: '/api/method//api/method/drive.api.files.get_translate',
  cache: 'translate',
})

export const storageBar = createResource({
  url: '/api/method/drive.api.storage.storage_bar_data',
  method: 'GET',
  cache: 'total_storage',
})
